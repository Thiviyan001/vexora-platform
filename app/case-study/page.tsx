...
export default function CaseStudy() {
  return <main className="case-page">
    <aside className="case-sidebar"><Link href="/" className="sidebar-brand">← VEXORA</Link><p className="sidebar-kicker">Case study</p><h2>The VEXORA Story</h2><p className="sidebar-copy">A brief guide through the 20-part presentation.</p><nav className="toc">{chapters.map(([number,title])=><a href={`#slide-${number}`} key={number}><span>{number}</span>{title}</a>)}</nav><small>VEXORA · Project preview</small></aside>
    <div className="case-content">
      <section className="case-hero section-pad"><Link href="/" className="back-link">← Back to VEXORA</Link><p className="eyebrow">VEXORA / Case Study</p><h1>From the problem<br/>to the <em>possibility.</em></h1><p>A concise case study of the ideas behind VEXORA — why it should exist, what it is trying to change, and where the project could go.</p><div className="case-chips"><span>20-part presentation</span><span>Project in development</span></div></section>
      ...
      {groups.map(([id,range,title,start,end])=><section className={`chapter-group ${id}`} key={id}><div className="group-intro"><span>{range}</span><h2>{title}</h2></div><div className="chapter-list">{chapters.filter(([number])=>{const n=Number(number);return n>=start&&n<=end}).map(([number,title,description])=><article id={`slide-${number}`} className="chapter" key={number}><span className="chapter-number">{number}</span><div><h3>{title}</h3><p>{description}</p></div><span className="chapter-arrow">↗</span></article>)}</div></section>)}
